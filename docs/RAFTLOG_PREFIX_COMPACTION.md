...
On open, the directory lock is acquired before stale `raft.log.tmp` is discarded. A temporary file is never promoted to authority. If a temporary file exists without `raft.log`, open fails and preserves the files for recovery rather than inventing an empty log. An incomplete source WAL tail causes compaction to fail without modifying it; invoke `replayLog()` to repair the torn tail first, then retry compaction. A source WAL with corruption inside the committed region fails with `CorruptLogException` and fences the instance; restore the node from its peers.
...
Release validation on 2026-09-05: Windows/JDK 25 `clean install` ran 319 tests with zero failures/errors and three existing platform skips. A non-root Linux container with JDK 21 ran `clean verify`: all 319 tests passed with no skips, including actual directory force and child-process interruption. A downstream consumer's five selected storage/snapshot/restart suites passed all 41 cases against the locally installed 1.2.0 artifact. This validates the integration boundary, not that consumer's entire reactor or deployment power-loss acceptance.
