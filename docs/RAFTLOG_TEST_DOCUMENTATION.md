...
- `GoldenFileCompatibilityTest` (27 cases) and `src/test/resources/golden`: data directories written by the published 1.1.0 and 1.2.0 jars, downloaded from Maven Central and verified against their published checksums, never rebuilt from source. `GoldenFileGenerator.java.txt` beside the fixtures is the program that produced them and the instructions for adding a release. Each scenario carries `expected.txt`, which is what that old release itself replays from a copy of the files, so the oracle is the old build's behaviour and not an expectation of it. Contract one: a well-formed directory replays exactly as the old release replayed it, payload bytes included, keeps its metadata, is not rewritten by replay, can be continued, and survives a restart. Contract two: a log those releases wrongly accepted (a gap, a duplicate index, a term regression) is refused without being modified; this is the deliberate break, pinned so it stays deliberate. Fixtures are copied before use, because the storage locks the directory and may repair a torn tail. `SHA256SUMS` plus a `.gitattributes` in that directory guard against a checkout rewriting line endings inside a binary fixture. Generate fixtures with every release from now on; without them, old logs still replaying is only an assumption.
...
- **The write limit is not a read limit** (in `FileRaftStorageFailurePathTest`): lowering `maxPayloadSizeMb` below the size of an entry already in the log used to make a healthy log replay as corrupt. A record being read is now bounded by the file that holds it, and the CRC decides whether it is genuine. The limit still applies to new writes, and still serves as a plausibility check on torn writes.
...
- `WalChaosTest` (7 cases, in `raftlog-demo`): runs the chaos suite as part of the build. `WalChaos` used to be only a program that somebody had to remember to launch, so the build could be green while every chaos scenario was failing, and the demo module sat at 16% line coverage against a 70% rule that nobody saw fail. `WalChaos.run(category)` now returns a summary, and `main` only turns it into an exit code. The test runs each category and the whole suite, requires zero failures, and pins the number of scenarios per category (6, 8, 9, 5 and 10, which is 38), so a scenario that is deleted or silently stops being registered fails the build. An unknown category throws, because running nothing and reporting success is worse than refusing. The `nasty` category, previously reachable only through `all`, can now be run on its own.
